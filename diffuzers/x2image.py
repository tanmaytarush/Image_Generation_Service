...
class X2Image:
    ...
    def __post_init__(self):
        ...
        if self.device == "cuda":
            ...
            _ = self.text2img_pipeline(prompt, num_inference_steps=4)
    ...
    def text2img_generate(
        self, * , prompt, negative_prompt, room_type, style, colors, furniture, scheduler, image_size, num_images, guidance_scale, steps, seed
    ):
    ...
    def img2img_generate(
        self, * , image, prompt, negative_prompt, scheduler, num_images, guidance_scale, steps, seed, strength
    ):
    ...
    def app(self):
        ...
        room_type_options = ["Bedroom", "Bathroom", "Balcony", "Living Room", "Spa", "Bar Room"]
        style_options = ["contemporary", "bohemian", "mid-century", "coastal"]
        colors_options = ["mood_setting", "space_perception", "coordination", "color_perception"]
        furniture_options = ["sofa", "centre_table", "dining_table", "wall_painting", "dressing_table", "cupboard"]

        room_type = st.selectbox("room_type", room_type_options)
        #if len(room_type) != None:
        style = st.selectbox("style", style_options)
        #if len(style) != None:
        colors = st.selectbox("color_textures", colors_options)
        #if len(colors) != None:   
        furniture = st.multiselect("furniture", furniture_options)
        #if len(furniture) != None:

        prompt_text_field = f"Generate an image of a {room_type} with style as {style} having a color texture of {colors} and list of furnitures {','.join(furniture)}"

        col1, col2 = st.columns(2)
        with col1:
            prompt = st.text_area("Prompt", prompt_text_field, help="Prompt to guide image generation")
        with col2:
